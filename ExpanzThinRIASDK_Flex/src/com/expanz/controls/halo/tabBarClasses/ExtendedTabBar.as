package com.expanz.controls.halo.tabBarClasses
{
	...
	import com.expanz.controls.halo.TabNavigatorEx;
		...
		**/
		...
		private function handleListDataItemClick(event:MenuEvent):void{
			...
			if (getChildAt(event.index).visible==true){
				...
				if (owner is TabNavigatorEx)
				{
					TabNavigatorEx(owner).setChildIndex(owner.getChildAt(event.index),0);
					TabNavigatorEx(owner).selectedIndex = 0;
				}							
			}
		}
